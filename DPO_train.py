...
model_type = f'EiRA_DPO_sft{fine_tuning_num}'
...
def main():
    ...
    try:
        ...
        # 获取包装后的模型
        policy_model = get_wrapped_model(policy_model, model_path=config.lora_checkpoint_path, update=True)
        ref_model = get_wrapped_model(ref_model, model_path=config.lora_checkpoint_path)
